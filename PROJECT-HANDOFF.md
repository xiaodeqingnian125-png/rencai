...
## 2026-07-20 MVP 上线增量（优先阅读）

> 下方 2026-07-19 快照保留作历史参考；其中分支、提交、页面数量、Mock 写操作和管理员模块状态已经过时。本节为当前增量结论。

### 当前代码状态

- 分支：`feat/mvp-launch`；当前已提交基线：`78f8f17`，本轮业务实现仍在工作区等待整体提交。
- 页面：25 个，新增 `pages/apartment-plans/index`。
- 数据模式：`miniprogram/data/db.js` 为 `cloud`。
- 自动化：30 条测试全部通过；全部 JS 语法、29 个 JSON、25 个页面四件套、差异格式通过。
- 微信官方编译：未验证。CLI 已获准开启服务端口，但工具未写出 `.ide` 端口文件并最终超时。
- 云端：本轮未重新部署云函数、未修改线上数据库、未上传体验版。

### 已实现核心

- 公寓/户型管理员 CRUD 全量调用受 `requireAdmin` 保护的云函数接口。
- 手工保存校验公寓编号唯一性；户型校验所属公寓并回填权威数字 `apartment_id`。
- 地址自动解析坐标；腾讯单次 3 秒，失败回退高德 3 秒；单次最多 10 条无坐标公寓，解析失败行不进入预览或写入。
- 管理员上传公寓/户型封面；失败保留旧图；图片限制 5MB。
- 公寓平面图保存为 `floor_plans: [{name,image}]`，详情前 3 张，独立页面展示第 4 张及以后。
- 公寓 CSV 导出封面路径和平面图 JSON；户型 CSV 导出封面路径；旧“封面图文件名”列仍可导入。
- 公寓/户型公开详情已移除所有管理员图片和经纬度维护控件。

### 预览模块

服务、活动、借物、室友、消息、收藏、评论、社群和个人业务页面保留可见并标记示例内容。报名、发布、下单、借用、消息删除/确认等动作只显示统一预览提示，不写数据、不显示假成功。登录、退出、个人设置入口和管理员公寓/户型管理保持真实。

### 部署前硬门槛

- [ ] 微信开发者工具手动开启服务端口并完成无错误编译。
- [ ] 部署 `rencai` 云函数时选择“云端安装依赖”。
- [ ] 确认云环境、腾讯/高德 Key、`ADMIN_OPENIDS` 和 `users.role`。
- [ ] 确认集合及索引：`users`、`apartments`、`room_types`、`import_tasks` 等。
- [ ] 数据库权限禁止普通小程序端直接写核心集合，写操作只经云函数。
- [ ] 管理员与普通用户各完成一轮权限验收。
- [ ] iPhone 小屏与大屏验证平面图、管理员长表单、地图和底部导航。
